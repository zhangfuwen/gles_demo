...
int main() {
    int fd = open("/sys/kernel/debug/kgsl/kgsl-3d0/profiling/assignments", O_RDWR);
    if(fd < 0) {
        printf("failed to open file, %d, %s\n", fd, strerror(errno));
        return errno;
    }

    char * str = "cp:2 rbbm:10 vsc:0 vsc:4";

    if(auto ret = write(fd, str, strlen(str)+1); ret < 0) {
        printf("failed to write str:%s, %s\n", str, strerror(errno));
        return errno;
    } else {
        printf("write assignments ok %d\n", ret);
    }
